...
undo_available_X = True  # One-time undo powerup for player X
undo_available_O = True  # One-time undo powerup for player O
moves_made = []  # List to keep track of all the moves made in the game
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if 0 <= row < 3 and 0 <= col < 3 and board[row][col] == "":
                # Record the move
                moves_made.append((turn, row, col, [r.copy() for r in board]))
                # Update the board
                board[row][col] = turn
                turn = "X" if turn == "O" else "O"
        elif event.type == pygame.KEYDOWN and event.key == pygame.K_u:  # Press "U" to undo
            if moves_made and ((turn == "O" and undo_available_X) or (turn == "X" and undo_available_O)):
                # Revert the last move
                last_move_turn, last_move_row, last_move_col, last_board_state = moves_made.pop()
                board = last_board_state
                turn = last_move_turn  # Switch back to the previous player's turn
                if turn == "X":
                    undo_available_X = False  # Player X used undo
                else:
                    undo_available_O = False  # Player O used undo
    ...
    if winner:
        ...
        if winner != "Draw":
            text = font.render(winner + " Wins!", True, (255, 255, 255))
        else:
            text = font.render("It's a Draw!", True, (255, 255, 255))

        # Create a black background box for the text
        text_background = pygame.Surface((text.get_width() + 20, text.get_height() + 20))
        text_background.fill((0, 0, 0))
        window.blit(text_background, (400 - text_background.get_width() // 2, 400 - text_background.get_height() // 2))
